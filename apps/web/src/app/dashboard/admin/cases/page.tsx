...
export default function AdminCasesPage(){ const [items,setItems]=useState<CaseItem[]>([]); useEffect(()=>{api.listCases().then(r=>{if(r.success&&r.data)setItems(r.data);});},[]); return <AuthGuard allowedRoles={["ADMIN"]}><DashboardShell breadcrumbs={[{label:"Dashboard",href:"/dashboard/admin"},{label:"Case Monitoring"}]}><PageContainer><SectionHeader title="Case Monitoring" description="Monitor all active and closed cases" />{items.length===0?<EmptyState title="No cases" description="No cases have been opened yet."/>:<Table><TableHeader><TableRow><TableHead>Case #</TableHead><TableHead>Title</TableHead><TableHead>Status</TableHead><TableHead>Priority</TableHead></TableRow></TableHeader><TableBody>{items.map(c=><TableRow key={c.id}><TableCell>{c.case_number}</TableCell><TableCell>{c.title}</TableCell><TableCell><Badge>{c.status?.display_name ?? "-"}</Badge></TableCell><TableCell>{c.priority}</TableCell></TableRow>)}</TableBody></Table>}</PageContainer></DashboardShell></AuthGuard>; }
