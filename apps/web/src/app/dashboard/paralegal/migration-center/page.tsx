...
export default function CaseMigrationCenterPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (q.success && q.data) setQueue(q.data as MigrationItem[]);
    if (s.success && s.data) {
  ...
  const actions = [
    ...
    { id: "bulkDocs" as Panel, label: "Bulk Import Documents", icon: FileUp },
                ...
                <Button
                  ...
                  onClick={() =>
                    run(async () => {
                      ...
                      if (!r.success) throw new Error(r.message);
                      setMessage(r.message || "Documents queued");
                      setPanel(null);
                    })
                  }
                >
                  ...
                  {queue.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={5} className="text-zinc-500">
                        No migration records yet. Run migration SQL 017–018 on Supabase first.
                      </TableCell>
                    </TableRow>
                  ) : (
                    queue.map((row) => (
